...
const PeopleStruggles = () => {
  const struggles = [
    ...
    },
    {
      ...
      title: "Team Fear",
      description:
        "My team fears AI will replace them, instead of empower them.",
    },
    {
      icon: GraduationCap,
      title: "Skills Gap",
      description:
        "Freshers join us, but most of them lack the right AI skills.",
    },
    {
      icon: XCircle,
      title: "No Time",
      description:
        "I know AI is important, but I don't have time to figure it out.",
    },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 max-w-7xl mx-auto">
          {struggles.map((struggle, index) => {
            ...
            return (
              <Card
                key={index}
                className="hover:shadow-lg transition-all duration-300 hover:-translate-y-2 border-red-100 group"
              >
                <CardContent className="p-6 text-center h-full flex flex-col">
                  <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-red-100 transition-colors duration-300">
                    <Icon className="w-8 h-8 text-red-500 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300" />
                  </div>
